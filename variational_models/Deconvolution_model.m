...
%---------------------------- Visualization ------------------------------%
    switch Verbose
        ...
        case 2
            ...
            subplot(142), plot(en,'r','LineWidth', 2), hold on,plot(fi,'b','LineWidth', 2),plot(pr,'g','LineWidth', 2)
            legend('Total Energy','Fidelity','Prior'), grid on
            subplot(143), plot(psnr,'c','LineWidth', 2), legend('PSNR (db)'), grid on
...
%----------------------------- Algorithm ---------------------------------%
    ...
    % Stop criteria
    disp(['Stop Criteria: ',num2str(norm(u2(:) - u(:)))])
    stop = norm(u2(:) - u(:)) < epsilon;
